...
defaultData = pd.read_sql("SELECT \"movieId\", MIN(title) AS title, MIN(adult) AS adult, MIN(release_date) AS release_date, string_agg(DISTINCT production_studio, ', ') AS production_studios, string_agg(DISTINCT language, ', ') AS languages, string_agg(DISTINCT genres, ', ') AS genres, ROUND(AVG(rating),2) * 2 as user_score, ROUND(AVG(vote_average),2) AS tmdb_score FROM searchTableRaw GROUP BY \"movieId\" ORDER BY title;", DATABASE_URL)
...
def update_graph(nClicks, keywords, genre, language, years):
    ...
    if conditionalExists:
        ...
        SELECT
            ...
            ROUND(AVG(rating),2) * 2 as user_score, 
            ROUND(AVG(vote_average),2) AS tmdb_score
        FROM searchTableRaw
        {conditionalSentence}
