...
Options:
    ...
    --cluster=<cluster_id>  Cluster Identifier (e.g. --cluster=active-roach-42)
"""
...
def build_sqla_connection_string(cockroach_cloud_url, timeout=10):
    ...
    if (
        ...
        raise ValueError("Was expecting connection string to start with "
                          f"'postgres://' but instead found {cloud_parse.scheme}://"
                          )
    ...
    sqla_url = (f'{scheme}://{username}:{password}@{hostname}:{port}/'
                f'{database}?{querystring}')
...
def build_querystring(query, connect_timeout=10):
    ...
    return query + f'&connect_timeout={connect_timeout}'
